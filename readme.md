...
>
> 本仓相对上游只有四处个人化改动，都只是**默认值/配置层面**的取舍，改回上游行为随时可以：
>
> | # | 改动 | 上游默认 | 本仓默认 | 怎么改回去 |
> | --- | --- | --- | --- | --- |
...
> | ④ | 录音期间静音系统输出，录完自动恢复（见[录音时静音](#录音时静音系统输出)） | 无 | 开启 | `mute_while_recording = False` |
>
...
-   **录音时静音**（本仓新增）：按下录音键自动静音系统输出，松开恢复。说话时不用先去暂停音乐或视频，也少了外放声串进麦克风。
...
`mute_while_recording = True`（本仓默认开启）：按下录音键时静音系统主音量，录音结束自动恢复。省得说话前先手动去暂停音乐或视频，也避免外放的声音串进麦克风。

几个细节：

-   静的是**整机输出**，不是逐个应用——包括系统提示音。想改成只压低别的应用音量，得改用会话级音量控制，本仓没做。
-   读写的是静音开关本身（`SetMute`），不是模拟按静音键做切换。所以**你原本就静音时它不会动**，录完也不会替你把声音打开。
-   客户端进程正常退出会兜底恢复；被任务管理器强杀则不会，按一下键盘静音键即可。
-   依赖 `pycaw`。没装的话功能自动跳过，只在日志里留一条警告，不影响录音。

不需要就把 `config_client.py` 的 `mute_while_recording` 改成 `False`。
...
**Q: 一按录音键音乐就没声了？**  
A: 这是默认行为，录完会自动恢复，见上面的[录音时静音](#录音时静音系统输出)。不想要就把 `config_client.py` 的 `mute_while_recording` 改成 `False`。如果录完声音没回来（通常是客户端被强杀了），按一下键盘静音键即可。
